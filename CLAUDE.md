...
- **Outlook live verification on a real tenant.** Code is verified by unit tests (52 inbound tests pass) and by direct OAuth `client_credentials` token acquisition. The remaining gap is end-to-end against a live shared mailbox. We were blocked on a Microsoft RBAC-for-Applications bootstrap problem in a fresh M365 Business Basic trial tenant (`MindLab168.onmicrosoft.com`): even a Global Admin can't self-grant delegation rights on the new `Application Mail.ReadWrite` / `Application Mail.Send` roles, so `New-ManagementRoleAssignment -App ...` always returns "you must be assigned a delegating role assignment". The Application Access Policy was created and `Test-ApplicationAccessPolicy` returned `Granted`, but Graph still serves `[RAOP] Blocked by tenant configured AppOnly AccessPolicy settings` because RBAC for Applications overrides legacy AppAccessPolicy in tenants created 2024+. Resume this when running against Lofty's actual production tenant — their IT team will own the bootstrap and won't hit the same self-delegation wall. See `docs/integrations.md § Setting it up against live Outlook` for the full operational checklist.
